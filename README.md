...
- [配置项](#配置项)
- [状态判定与去抖语义](#状态判定与去抖语义)
- [管理员测试通知（`/botbrother_test`）](#管理员测试通知botbrother_test)
- [常见问题](#常见问题)
...
- 管理员测试通知：`/botbrother_test` 一键验证通知渠道配置，无需被监视账号在线，也不改动监视状态。
...
## 管理员测试通知（`/botbrother_test`）

在任意已接入的平台会话中，由**管理员**发送指令 `/botbrother_test`，即可向**当前配置的 `notify_targets`** 各发送一条测试通知，并返回逐目标结果摘要。

用途与边界：

- 用于验证通知渠道配置是否可用，**不代表被监视账号的状态变化**，不会改动状态机/快照，也不会触发任何故障告警。
- **仅管理员可用**：注册时带管理员权限过滤器，处理器内还会再做一次管理员复核；非管理员发送会被拒绝。
- **只能发给已配置的 `notify_targets`**，指令不接受、也无法指定任意收件人。
- 不依赖被监视 QQ 账号在线，也不要求监视任务已启动或配置完全合法：只要 `notify_targets` 本身配置合理即可测试（例如 `enabled=false` 或 `platform_id` 尚未填好时也能验证渠道）。
- 测试文案固定为：`[BotBrother] 测试通知：用于验证通知渠道配置，不代表账号状态变化。`
- 每个目标独立发送、单个目标 15 秒超时，失败/超时只影响该目标，不阻塞其余目标；重复/并发触发时后一次会提示“已有测试通知正在进行”，不叠加发送。

返回摘要的状态含义：

| 状态 | 含义 |
| --- | --- |
| 发送调用完成（请核对收件） | 已调用适配器发送，且目标平台已定位（`send_message` 返回真值）。**AstrBot 该返回值只表示“找到匹配平台”，不代表消息真实可达、用户已读或最终投递成功；请核对收件并查看适配器响应/日志确认。** |
| 未找到目标平台 | 目标平台实例不存在或未启用（`send_message` 返回 False）。 |
| 超时 | 单目标发送调用超过 15 秒未返回。 |
| 失败 | 发送调用抛出异常（摘要只显示异常类型，详情见日志，避免泄露 token/URL）。 |

摘要末尾给出 `发送调用完成=…，超时=…，失败=…，共 N 个目标` 的计数。

> 提示：`send_message` 返回真值仅确认平台已匹配；远端部署是否真正送达，需结合目标适配器的发送响应与运行日志确认。
