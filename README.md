...
- 告警尽量不丢：发送与探测解耦（独立 worker），失败目标按每目标 FIFO 入队并指数退避重试（有界）；不改变告警文案与去抖语义。
...
- 摘要中每个目标附带诊断 id（形如 `[id=xxxxxxxx]`），可在日志中检索该次发送的开始/耗时/状态；测试发送**不入队、不改退避计数**，与告警补发队列互不影响。
...
### 发送与探测解耦（待发送队列 + 通知 worker）

- 探测循环与平台活动事件**只把告警写入队列**，不做任何网络发送；发送由独立、生命周期受管的通知 worker 完成，因此发送挂起/失败不会拖慢探测与其它目标。
- worker 特性：有限并发（≤3）、**每目标 FIFO**（旧通知未成功前不越级）、指数退避（失败次数先截断防溢出，封顶 1 小时）、每轮尝试上限；单目标超时 15s，坏目标不会长期饿死健康目标。
- 队列文件：`data/plugin_data/astrbot_plugin_botbrother/pending_notifications.json`。**始终外置到 data 目录**，与 `persist_state` 无关（`persist_state` 只控制 `state.json` 快照；data 目录不可用则降级为内存并记警告）。
- 成功目标移除、失败目标保留退避、配置中删除的目标清理；磁盘写失败只记日志（内存保留，该次持久化丢失）；损坏类型/非有限值/巨大数严格回退。
- worker 可取消：`terminate` 会取消并等待 worker 后保存队列，无任务泄漏。**崩溃窗口**（发送已成功但尚未写盘移除）可能导致重启后重复发送一次，因此**不承诺 exactly-once**。
- 容量上限 50 条，超限丢弃最旧并记录日志；队列是“尽量不丢 + 有界重试”，**不承诺绝不丢失**，也不等于及时预警。
- **成功语义**：从队列移除仅代表适配器发送调用返回（目标平台已定位），**不代表用户已读或最终投递**。
- 仅“故障告警/恢复通知”入队；管理员 `/botbrother_test` 的测试通知不入队，保持即时反馈。
- 诊断：每次发送输出关联 id、目标、平台实例、尝试次数、开始/结束耗时、状态/异常类型；失败详情在日志，不输出凭据或完整请求/token。适配器的原始 `ret/errmsg` 无法经公开 API 获取，只能按时间/平台关联适配器日志。

### 微信个人号（weixin_oc）主动发送能力（结论更正）

- **weixin_oc 支持主动发送**：定时任务、agent 工具与本插件都走同一条 `Context.send_message → platform.send_by_session` 路径，插件不自建微信适配器、不使用 requests、不伪造/刷新 token。
- 已知事实：适配器发送时要求存在缓存的 `context_token`，且仅在收到该用户入站消息时刷新。但此前“`prepare failed` = token 过期”的结论**已撤回**，**原始失败原因仍未证实**。
- 因此：**不要求用户定期发消息**，也不把“必须更换渠道”作为结论。若希望降低单点风险，可自行额外配置一个 AstrBot 支持的独立渠道（可选、非强制）。
- 本插件的补发队列是**可靠性加固，不等于上述失败的根因已解决**。
