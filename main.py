"""astrbot_plugin_botbrother —— 单实例 NapCat/OneBot 在线监视插件。
...
- 推送一律走 AstrBot Context/平台 API（``context.send_message``）；目标可为 AstrBot 中任意平台的会话（unified_msg_origin），不限于被监视的 OneBot 实例。逐目标独立尝试，单个目标失败仅记录日志，不影响其余目标。
- 后台任务可取消、无泄漏；所有异常仅记录，不令插件崩溃。
- 持久化（可选）写入 AstrBot 的 data/plugin_data/astrbot_plugin_botbrother/。
"""
...
from astrbot.core.star.register import register_platform_adapter_type
...
class BotBrotherMonitor(Star):
    ...
    def __init__(self, context: Context, config: AstrBotConfig) -> None:
        ...
        self._config: dict = {}
        self._platform_id = ""  # 启动后由配置 platform_id 赋值（AstrBot Bots 中的平台实例 ID）
    ...
    async def _push(self, text: str) -> None:
        ...
        unified_msg_origin（不限于被监视的 OneBot 实例）；逐目标独立尝试，
        单个目标失败仅记录日志，不影响其余目标。
        """
        ...
        for target in targets:
            try:
                ok = await self.context.send_message(target, chain)
                if not ok:
                    logger.warning(
                ...
                logger.error(
                    "%s 推送失败 target=%s（已忽略）：%s", PLUGIN_NAME, target, e
                )
    ...
    def _load_snapshot(self) -> None:
        ...
        try:
            ...
            logger.warning("%s 状态快照加载失败，使用全新状态（已忽略）：%s", PLUGIN_NAME, e)
